export function evaluateCredentialEligibility({ credential, evidence = {} } = {}) {
  ...
  for (const requiredId of credential.eligibility.requiredAssessments ?? []) {
    ...
    if (!result) { missing.push({ type: 'assessment', id: requiredId, reason: 'missing-result' }); continue; }
    if (result.status !== 'passed') { missing.push({ type: 'assessment', id: requiredId, reason: 'not-passed' }); continue; }
  ...
  for (const requiredId of credential.eligibility.requiredPerformanceAssessments ?? []) {
    ...
    if (!result) { missing.push({ type: 'performance-assessment', id: requiredId, reason: 'missing-result' }); continue; }
    if (result.status !== 'passed') { missing.push({ type: 'performance-assessment', id: requiredId, reason: 'not-passed' }); continue; }
  ...
  for (const requiredId of credential.eligibility.requiredPortfolioArtifacts ?? []) {
    ...
    if (!result) { missing.push({ type: 'portfolio-artifact', id: requiredId, reason: 'missing-artifact' }); continue; }
    if (!['accepted', 'verified', 'complete'].includes(result.status)) missing.push({ type: 'portfolio-artifact', id: requiredId, reason: 'artifact-not-accepted' });
  }
  return {
