...
| Downloads / assets | Twenty-three draft CSV learner tools, a searchable Academy Downloads view, course visual registries, and controlled asset routes | Download schema, Technician I seven-tool pack, Technician II eight-tool pack, four-tool SOP learner pack, global registry mapping, safe metadata API, accessible UI states, file routes, and production visibility boundary are deterministic | Templates need instructional/accessibility review before public release | External controlled SOP packages cannot be represented until their source files are available | `downloads:test`, `tech1:downloads:test`, and `sop:ingestion:test` verify metadata, mappings, blank files, authority boundaries, routes, privacy, responsive rules, traversal rejection, and draft isolation |
...
| Missing / machine work | Controlled-source ingestion for the 24 SOP targets, production issuer identity/signing/revocation persistence, deployed practical-submission validation, and exact deployment identity records |
