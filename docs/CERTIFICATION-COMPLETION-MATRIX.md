...
| Downloads / assets | Twenty-three draft CSV learner tools, a searchable Academy Downloads view, controlled asset routes, **23 reviewed PNG learner assets for Technician I Course 1**, and **36 owner-approved lossless WebP primary visuals for Technician II**. Legacy SVG files remain compatibility/provenance sources only. | Course 1 production raster cutover is active; Technician II 36/36 primary visuals are wired to the approved WebP manifest; raster-only production policy and deterministic integrity checks are active | Remaining visual work is course-specific manual accessibility/responsive/public-runtime QA and any future raster expansion for courses that still need more instructional media; do not reclassify legacy SVGs as production assets | External controlled SOP packages cannot be represented until their source files are available | `tech2:raster-wiring:test`, Course 1 visual registries, raster policy checks, and public asset existence checks prevent metadata from drifting back to SVG-as-production |
...
3. Preserve the current raster cutover: Course 1 uses reviewed PNG instructional assets and Technician II uses 36/36 approved WebPs; expand remaining course visuals only where instruction benefits, never by reverting SVGs to production status.
