...
| Downloads / assets | Twenty-three draft CSV learner tools, a searchable Academy Downloads view, course visual registries, controlled asset routes, and 96 governed raster replacement candidates | Download schema, support packs, registry mapping, safe routes, raster-only policy, candidate integrity checks, and production-candidate generation are deterministic; all 96 governed raster candidates now exist and Course 1 has released 20/20 replacements | The remaining 76 Technician I/II raster candidates are fail-closed pending human factual/copy, authority-boundary, accessibility, responsive and verified cutover review; no remaining raster binary production is open | External controlled SOP packages cannot be represented until their source files are available | Raster backlog reporting and regression tests prevent visual policy from silently drifting back to SVG-as-production; source/candidate SHA-256, dimensions, encoding and byte counts are recorded |
...
| Missing / machine work | Controlled-source ingestion for remaining SOP targets, production issuer identity/signing/revocation deployment, deployed practical-submission validation, production DB/RLS/MFA deployment, backup/restore validation, and monitoring evidence |
...
| THC Cultivation Technician II | Professional credential | Visible | All 8 public academic packages present; 36 governed raster candidates are produced and remain in controlled human review; human/pilot/credential gates remain | Blocked |
...
3. Complete human QA and verified cutover for the remaining 76 raster candidates; all 96 governed raster binaries are now produced, Course 1 has released 20/20 replacements, and SVG compatibility/review assets cannot be retired until each replacement passes its release gate.
