...
| Certification logic | 11 credentials; Tech I/II pathways modeled | Eligibility, progress, transcript, test issuance, verification tooling exists | Production issuer identity/signing/revocation persistence not ready | Practical evidence submission workflow is still a production blocker | Credential coverage reports complete 24/26 credential-bearing paths; C7/C8 are intentionally draft-gated |
...
| Missing / machine work | Remaining SOP package ingestion, production persistence adapter, production issuer identity/signing/revocation persistence, practical evidence submission workflow, exact deployment identity records |
