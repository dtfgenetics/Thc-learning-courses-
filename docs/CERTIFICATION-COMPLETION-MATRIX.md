...
| Downloads / assets | Twelve draft CSV learner tools, a searchable Academy Downloads view, course visual registries, and controlled asset routes | Download schema, Technician II eight-tool job-aid pack, global registry mapping, safe metadata API, accessible UI states, file routes, and production visibility boundary are deterministic | Templates need instructional/accessibility review before public release | SOP packages and additional Technician I printable packs are not yet represented | `npm run downloads:test` verifies metadata, UI contract, files, routes, responsive rules, traversal rejection, and draft isolation |
...
| Missing / machine work | Remaining SOP package ingestion, Technician I printable job-aid pack, production issuer identity/signing/revocation persistence, deployed practical-submission validation, and exact deployment identity records |
...
2. Keep Technician I's seven machine-complete source packages and seven canonical performance objects green while building its printable job-aid pack and closing exact deployment identity, responsive/manual QA, raster asset replacement, and changed-object review evidence.
