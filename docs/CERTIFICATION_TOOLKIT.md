...
The worker reads `registry/system-readiness.json`, identifies unresolved gates in priority order, and selects the next action without creating duplicate work when an active branch or PR already owns the same gate.

```bash
npm run worker:next
npm run worker:next:json
node scripts/certification-production-worker.mjs --active-work=path/to/active-work.json
```

The worker does not mark gates complete. It only turns the readiness registry into a deterministic work queue. Human/scientific/editorial approvals remain evidence-backed release requirements.
...
Each toolkit check records its group, npm script name, exit status, duration, signal, and spawn error. The final summary includes total checks, pass/fail totals, failed script names, total duration, and whether every selected check executed.
