...
try {
  ...
  for (const asset of producedAssets) {
    assert.match(asset.learnerPath ?? '', /^\/assets\/course4\/[A-Za-z0-9._-]+\.svg$/, `${asset.id} should use a controlled Course 4 learner path`);
  ...
  const invalidCourseDirectory = await fetch(`${base}/assets/course4x/sample-ph-ec-context.svg`);
  assert.equal(invalidCourseDirectory.status, 404, 'course asset routing must only accept course<number> directories');
  const missingAsset = await fetch(`${base}/assets/course4/not-a-real-asset.svg`);
