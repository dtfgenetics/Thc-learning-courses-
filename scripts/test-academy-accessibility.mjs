...
const credentialVerification = fs.readFileSync('apps/web/public/credential-verification.js', 'utf8');
...
expect(/credential-verification\.js/i, html, 'employer lifecycle verification enhancement must load');
...
expect(/\.portal-verification-section\s*\{/i, portalStyles, 'employer lifecycle sections need dedicated readable layout');
expect(/@media\s*\(max-width:\s*620px\)[^}]*portal-credential-card dl/i, portalStyles, 'employer verification details need single-column narrow-screen treatment');
...
expect(/document\.createElement\('ol'\)/, credentialVerification, 'credential status history must use ordered-list semantics');
expect(/document\.createElement\('dl'\)/, credentialVerification, 'credential version and lifecycle facts must use description-list semantics');
