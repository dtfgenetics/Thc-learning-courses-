...
for(const concept of manifest.concepts.slice(12,18)){
  ...
  assert.equal(candidate.binaryState,'repository-production-master-human-qa-required');
  assert.equal(candidate.qaStatus,'production-master-review-candidate');
  ...
  assert.equal(concept.releaseApproved,false);
...
console.log('Course 1 production masters 13–18: PASS (six 2400×3200 PNG review masters; learner cutover remains human-QA gated).');
