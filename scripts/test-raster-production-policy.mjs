...
for(let n=2;n<=6;n++){
  ...
  if(n===2){
    assert.equal(registry.policy?.legacySvgCompatibilityAllowed,false,'Course 2: SVG must not remain an active learner compatibility path after raster cutover');
    assert.equal(registry.policy?.rasterReplacementRequired,false,'Course 2: raster replacement is complete');
    for(const asset of registry.assets??[]){
      assert.equal(path.extname(asset.sourcePath??'').toLowerCase(),'.webp',`${asset.id}: Course 2 production source must be WebP`);
      assert.equal(asset.assetLifecycle,'production-raster-active',`${asset.id}: Course 2 raster lifecycle drift`);
      assert.equal(asset.rasterReplacement?.candidateSourcePath,asset.sourcePath,`${asset.id}: active raster path must match released candidate path`);
      assert.match(asset.legacySource?.sourcePath??'',/^apps\/web\/public\/assets\/course2\/[A-Za-z0-9._-]+\.svg$/i,`${asset.id}: SVG provenance source required`);
    }
    continue;
  }
