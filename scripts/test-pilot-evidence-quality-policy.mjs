...
function record(overrides = {}) {
  return {
    ...
    percentCorrect:0.6, discrimination:{method:'point-biserial',value:0.25},
    distractorSelection:[
...
const negative = activationEvidenceEvaluation(record({discrimination:{method:'point-biserial',value:-0.1}}), item, policy);
if (negative.ready || !negative.blockers.includes('discrimination-below-activation-floor')) throw new Error('Negative discrimination must block activation.');
...
const lowDiscrimination = activationEvidenceEvaluation(record({discrimination:{method:'point-biserial',value:0.05}}), item, policy);
if (!lowDiscrimination.ready) throw new Error('Low-but-nonnegative discrimination is a review signal, not an automatic activation blocker.');
...
console.log(`Pilot evidence quality policy tests passed (activation floor ${policy.activation.minimumResponsesPerItem}, target ${policy.reviewSignals.targetResponsesPerItem}).`);
