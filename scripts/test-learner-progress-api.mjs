...
try {
  ...
  response = await fetch(`${base}/api/v1/me/lessons/LESSON-ENV-VPD-001`, {
    ...
    body: JSON.stringify({ lessonVersion: '1.1.0', status: 'completed' })
  });
  ...
  response = await fetch(`${base}/api/v1/me/lessons/LESSON-ENV-VPD-001`, {
    method: 'PUT',
    headers: { authorization: 'Bearer alice', 'content-type': 'application/json' },
    body: JSON.stringify({ lessonVersion: '1.1.0', status: 'passed' })
  });
  ...
  assert.equal(mismatch.currentVersion, '1.1.0');
