...
for (const asset of produced) {
  ...
  assert.match(asset.learnerPath ?? '', /^\/assets\/course5\/[A-Za-z0-9._-]+\.svg$/i);
  assert.match(asset.sourcePath ?? '', /^apps\/web\/public\/assets\/course5\/[A-Za-z0-9._-]+\.svg$/i);
...
console.log(`Course 5 learner-asset contract passed for ${produced.length} public, accessible, Drive-mirrored and lesson-reachable assets across all four lessons.`);
