...
const ADMIN_READ = 'test-read';
const ADMIN_WRITE = 'test-write';
...
let history = [
  { status: 'valid', reason: 'initial-validation', actorId: 'INTERNAL-SYSTEM', createdAt: '2026-09-01T12:01:00.000Z' },
  { status: 'suspended', reason: 'INTERNAL-INVESTIGATION', actorId: 'INTERNAL-ADMIN-A', createdAt: '2026-09-02T12:01:00.000Z' },
  { status: 'valid', reason: 'INTERNAL-CLEARED', actorId: 'INTERNAL-ADMIN-B', createdAt: '2026-09-03T12:01:00.000Z' }
];
...
try {
  ...
  for (const forbidden of ['INTERNAL-SYSTEM','INTERNAL-ADMIN-A','INTERNAL-ADMIN-B','INTERNAL-INVESTIGATION','INTERNAL-CLEARED','actorId','reason']) {
  ...
  assert.equal(historyResult.body.history[1].reason, 'INTERNAL-INVESTIGATION');
  assert.equal(historyResult.body.history[1].actorId, 'INTERNAL-ADMIN-A');
  ...
  const invalidStatus = await request('/api/v1/admin/credentials/VERIFY-TECH2-LIFECYCLE-001/status', {
  ...
  const invalidTransition = await request('/api/v1/admin/credentials/VERIFY-TECH2-LIFECYCLE-001/status', {
    method: 'POST', headers: { authorization: `Bearer ${ADMIN_WRITE}`, 'content-type': 'application/json' }, body: JSON.stringify({ status:'valid' })
  });
  assert.equal(invalidTransition.response.status, 409);

  const missing = await request('/api/v1/admin/credentials/VERIFY-NOT-FOUND/status', {
    method: 'POST', headers: { authorization: `Bearer ${ADMIN_WRITE}`, 'content-type': 'application/json' }, body: JSON.stringify({ status:'revoked' })
  });
...
console.log('Credential lifecycle API privacy, authorization, history, suspension and reinstatement tests passed.');
