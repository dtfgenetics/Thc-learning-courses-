...
assert.equal(register.summary.sourceMappedPlannedPackages,17);
...
for(const item of register.externalPackages){
  ...
  assert.equal(item.releaseState,'blocked',`${item.externalId}: inventory reconciliation must not imply release approval`);
...
console.log('SOP controlled-target inventory and Drive provenance boundary: PASS (24/24 target scopes identified; release remains fail-closed).');
