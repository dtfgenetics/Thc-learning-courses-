...
for (const item of register.externalPackages) {
  ...
  assert.equal(item.sourceState, 'not-present-in-repository');
  assert.equal(item.ingestionStatus, 'pending-source');
}
...
console.log('SOP learner pack and controlled-source ingestion boundary: PASS.');
