...
for(const courseId of canonical){
  ...
  const hasLearnerPackage=learnerCandidates.some(exists);
  const remediationSignals=(supportText.match(/remediat|corrective coaching|reassessment/gi)??[]).length;
  const jobAidSignals=(supportText.match(/worksheet|job aid|checklist|download|practice sheet|record template/gi)??[]).length;
  const integratedSignals=(supportText.match(/integrated (practice|scenario)|multi-stage scenario|application scenario/gi)??[]).length;
  ...
  rows.push({
    ...
    signals:{workedExamples,commonMistakes,practicalApplications,scenarios,images,resources,tables,remediationSignals,jobAidSignals,integratedSignals},
    flags
  });
}
...
if(process.argv.includes('--json')){
  ...
  console.log('| Course | Score | Lessons | Worked examples | Common mistakes | Applied practice | Scenarios | Images | Resources/job aids | Remediation | Flags |');
  console.log('|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|');
  for(const r of rows){
    console.log(`| ${r.courseId} | ${r.supportScore} | ${r.lessonCount} | ${r.signals.workedExamples} | ${r.signals.commonMistakes} | ${r.signals.practicalApplications} | ${r.signals.scenarios+r.signals.integratedSignals} | ${r.signals.images} | ${r.signals.resources+r.signals.jobAidSignals} | ${r.signals.remediationSignals} | ${r.flags.join(', ')||'—'} |`);
  }
}
