...
const registry = readJson('visuals/COURSE4-ASSET-REGISTRY.json');

assert.equal(registry.courseId, 'COURSE-LH-TECH1-004');
...
assert.ok(embedded.length >= 5, `Course 4 requires at least five embedded visuals; found ${embedded.length}`);
assert.ok(downloads.length >= 2, `Course 4 requires at least two downloadable practice assets; found ${downloads.length}`);
...
for (const lessonNumber of ['01', '02', '03', '04']) {
  const lesson = readJson(`content/lessons/LESSON-LH-TECH1-004-${lessonNumber}.json`);
  let imageCount = 0;
  ...
  imageCountByLesson.set(lesson.id, imageCount);
