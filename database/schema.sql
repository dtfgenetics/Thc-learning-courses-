...
create table if not exists learner_practical_submissions (
  learner_id uuid not null references learners(id),
  course_id text not null,
  assessment_id text not null,
  assessment_version text not null,
...
create index if not exists idx_practical_submission_course on learner_practical_submissions(course_id, assessment_id, status, updated_at desc);
