...
const COURSE_PRACTICAL_PUBLIC = {
  ...
  "version": "1.0.0",
  "status": "published",
  "deliveryModes": ["simulation", "supervised-lab", "workplace-equivalent"],
  "evidenceOutputs": ["Pre-task hazard brief", "Sanitation and biosecurity movement record", "Controlled-instruction review", "Traceability movement and reconciliation record", "Equipment readiness and fault report", "Corrected record preserving original history", "Professional shift handoff"],
  "scoring": {
    "totalPoints": 100,
    "domains": [
      {"name": "Safety, PPE and stop-work judgment", "points": 18},
      {"name": "Sanitation and biosecurity", "points": 14},
      {"name": "SOP and work-order execution", "points": 12},
      {"name": "Identity and traceability", "points": 16},
      {"name": "Inventory and discrepancy reasoning", "points": 10},
      {"name": "Equipment readiness and operator-care boundary", "points": 10},
      {"name": "Data integrity and documentation", "points": 12},
      {"name": "Shift handoff and communication", "points": 8}
    ]
  },
  "passingStandard": {"minimumPercent": 80, "noCriticalErrors": true},
  "criticalErrors": [
  ...
  "stages": [
    {"title": "1. Pre-task review", "summary": "Recognize hazards, access restrictions, work-order limits, identity risks, PPE needs, and operator authority before work begins."},
    {"title": "2. Status and sanitation change", "summary": "Reassess work when one restriction changes but another remains active, and verify exact sanitation-product authorization before substitution."},
    {"title": "3. Recurring equipment fault", "summary": "Stay inside the operator-care boundary, record the fault timeline, and escalate repeated unresolved conditions without unsupported diagnosis."},
    {"title": "4. Record integrity", "summary": "Correct records while preserving original history and distinguish observation time from later entry time without backdating."},
    {"title": "5. Closed-loop handoff", "summary": "Transfer unresolved holds, identity discrepancies, equipment status, sanitation status, and next actions using two-way read-back and source cross-checking."}
  ],
  "supportResources": ["Course 1 Field References", "Course 1 Student Workbook", "Course 1 Workbook Templates", "Course 1 remediation matrix and instructor coaching when assigned"],
...
function courseActionLabel(outcome) {
  ...
  if (outcome === 'not-passed' || outcome === 'passed') return 'Retake course final';
...
async function enhanceEvidencePanel(panel) {
  ...
  const practicalButton = el('button', 'Study course practical', 'course-assessment-secondary course-practical-launch'); practicalButton.type = 'button'; practicalButton.addEventListener('click', () => openCoursePractical(practicalButton)); actions.append(practicalButton);
  if (result.state === 'loaded') {
    const written = result.data?.writtenAssessment ?? {};
    const finalButton = el('button', courseActionLabel(written.outcome), 'course-assessment-launch'); finalButton.type = 'button'; finalButton.addEventListener('click', () => openCourseAssessment(finalButton)); actions.prepend(finalButton);
  }
  panel.append(actions);
}

function observeEvidencePanel() { const panel = catalogRoot?.querySelector('.course-evidence-panel'); if (panel) enhanceEvidencePanel(panel); }
function practicalStatusLabel(status) { return ({ passed: 'Passed', failed: 'Not passed', 'in-progress': 'In progress', 'not-recorded': 'Not evaluated', voided: 'Voided' })[status] ?? String(status ?? 'not-recorded').replaceAll('-', ' '); }

function renderPracticalPersonalStatus(panel, evidenceResult) {
  const section = el('section', '', 'course-practical-personal-status'); section.append(el('h3', 'Your assessor-recorded practical status'));
  if (evidenceResult.state === 'authentication-required') section.append(el('p', 'The practical is public academic content. Sign in only if you want to view your private assessor-recorded practical result.', 'course-assessment-note'));
  else if (evidenceResult.state !== 'loaded') section.append(el('p', 'Personal practical status is temporarily unavailable. The academic practical below remains fully viewable.', 'course-assessment-note'));
  else {
    const practical = evidenceResult.data?.performanceAssessment ?? { status: 'not-recorded' };
    const line = el('div', '', 'course-practical-status-line'); line.append(el('strong', practicalStatusLabel(practical.status), `course-evidence-status status-${practical.status ?? 'not-recorded'}`));
    const details = [];
    ...
    line.append(el('span', details.length ? details.join(' • ') : 'No scored practical result is recorded yet.')); section.append(line);
    if (practical.status === 'failed') section.append(el('p', 'Use the Course 1 workbook, Field References, and instructor remediation to practice the weak performance areas before an equivalent reassessment.', 'course-practical-remediation'));
    else if (practical.status === 'voided') section.append(el('p', 'This practical record is voided. Review the recorded next step with the instructor or assessor before another evaluated attempt.', 'course-practical-remediation'));
  }
...
function renderCoursePractical(evidenceResult) {
  setCurriculumTabActive(); const practical = COURSE_PRACTICAL_PUBLIC; const panel = el('article', '', 'portal-panel course-practical-panel');
  panel.append(el('p', 'Course 1 public academic practical', 'eyebrow'), el('h2', practical.title), el('p', practical.overview, 'lede'));
  const meta = el('div', '', 'course-practical-meta'); meta.append(el('span', 'Published', 'course-practical-pill')); for (const mode of practical.deliveryModes) meta.append(el('span', mode.replaceAll('-', ' '), 'course-practical-pill subtle')); panel.append(meta, el('p', practical.academicUse, 'course-assessment-note')); renderPracticalPersonalStatus(panel, evidenceResult);
  const prep = el('section', '', 'course-practical-section'); prep.append(el('h3', 'Preparation')); appendNumberedList(prep, practical.preparationSteps, 'course-practical-list'); panel.append(prep);
  const stages = el('section', '', 'course-practical-section'); stages.append(el('h3', 'Five-stage workflow')); const stageGrid = el('div', '', 'course-practical-stage-grid'); for (const stage of practical.stages) { const card = el('article', '', 'course-practical-stage'); card.append(el('h4', stage.title), el('p', stage.summary)); stageGrid.append(card); } stages.append(stageGrid); panel.append(stages);
  const evidence = el('section', '', 'course-practical-section'); evidence.append(el('h3', 'Seven required evidence outputs')); appendNumberedList(evidence, practical.evidenceOutputs, 'course-practical-list'); panel.append(evidence);
  const scoring = el('section', '', 'course-practical-section'); scoring.append(el('h3', '100-point scoring model')); const scoreGrid = el('div', '', 'course-practical-score-grid'); for (const domain of practical.scoring.domains) { const card = el('div', '', 'course-practical-score-domain'); card.append(el('strong', `${domain.points} pts`), el('span', domain.name)); scoreGrid.append(card); } scoring.append(scoreGrid); const pass = el('p', `${Number(practical.passingStandard.minimumPercent).toFixed(0)}% minimum score`, 'course-practical-pass'); if (practical.passingStandard.noCriticalErrors) pass.append(document.createTextNode(' • no critical errors')); scoring.append(pass); panel.append(scoring);
  const critical = el('section', '', 'course-practical-section course-practical-critical'); critical.append(el('h3', 'Critical-error boundaries'), el('p', 'These boundaries protect the integrity of the practical. They define actions that cannot be offset by points earned elsewhere.')); appendBulletList(critical, practical.criticalErrors, 'course-practical-list'); panel.append(critical);
  const resources = el('section', '', 'course-practical-section'); resources.append(el('h3', 'Study and preparation resources')); appendBulletList(resources, practical.supportResources, 'course-practical-list'); panel.append(resources, el('p', practical.boundary, 'course-assessment-note course-practical-boundary'));
  const actions = el('div', '', 'course-practical-page-actions'); const printButton = el('button', 'Print practical', 'course-assessment-secondary'); printButton.type = 'button'; printButton.addEventListener('click', () => window.print()); const referencesButton = el('button', 'Open Field References', 'course-assessment-secondary'); referencesButton.type = 'button'; referencesButton.addEventListener('click', () => document.querySelector('#tab-field-references')?.click()); const back = el('button', 'Return to Course 1', 'course-assessment-secondary'); back.type = 'button'; back.addEventListener('click', () => document.querySelector('#tab-catalog')?.click()); actions.append(printButton, referencesButton, back); panel.append(actions);
  lessonView.replaceChildren(panel); lessonView.focus();
}

async function openCourseAssessment(sourceButton) {
  sourceButton.disabled = true; const original = sourceButton.textContent; sourceButton.textContent = 'Opening final…';
  try {
    const response = await fetch(`/api/v1/me/courses/${COURSE_ID}/assessment-attempts`, { method: 'POST', headers: { accept: 'application/json' }, credentials: 'same-origin' });
...
function singleChoiceControl(item, choice, index, fieldset, panel) { const label = el('label', '', 'course-assessment-choice'); const input = document.createElement('input'); input.type = 'radio'; input.name = `final-${currentAttempt.attempt.id}-${item.id}`; input.value = String(index); input.checked = Number(item.response) === index; input.addEventListener('change', () => persistResponse(panel, item, fieldset)); label.append(input, el('span', choice)); return label; }
function multipleChoiceControl(item, choice, index, fieldset, panel) { const label = el('label', '', 'course-assessment-choice'); const input = document.createElement('input'); input.type = 'checkbox'; input.value = String(index); input.checked = Array.isArray(item.response) && item.response.includes(index); input.addEventListener('change', () => persistResponse(panel, item, fieldset)); label.append(input, el('span', choice)); return label; }
...
function renderAssessment(payload) {
  currentAttempt = payload; pendingSaves.clear(); saveChains.clear(); setCurriculumTabActive(); const panel = el('article', '', 'portal-panel course-assessment-panel'); panel.append(el('p', 'Course 1 summative assessment', 'eyebrow'), el('h2', payload.assessment.title), el('p', `This is the public Course 1 final, separate from the Technician I credential examination. Passing score: ${Number(payload.assessment.passingScorePercent).toFixed(0)}%.`, 'lede'), el('p', payload.resumed ? 'Your open attempt was resumed. Previously saved responses are restored.' : 'A new attempt has started. Responses save to your learner record as you answer.', 'course-assessment-note'));
  const toolbar = el('div', '', 'course-assessment-toolbar'); toolbar.append(el('strong', '0/0 answered', 'course-assessment-progress'), el('span', 'Responses saved.', 'course-assessment-save-status')); panel.append(toolbar);
  const form = document.createElement('form'); form.className = 'course-assessment-form'; form.addEventListener('submit', async (event) => { event.preventDefault(); await submitAssessment(panel); }); payload.items.forEach((item, index) => form.append(renderAssessmentItem(item, index, panel)));
  const submitArea = el('div', '', 'course-assessment-submit-area'); const submit = el('button', 'Submit course final', 'course-assessment-submit'); submit.type = 'submit'; submit.disabled = true; submitArea.append(submit, el('p', 'Submission is scored server-side. Post-attempt feedback is domain-level; answer keys are not displayed.', 'course-assessment-note')); form.append(submitArea); panel.append(form); lessonView.replaceChildren(panel); lessonView.focus(); updateAssessmentProgress(panel);
}
